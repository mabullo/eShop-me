<?php
/**
 * @package WordPress
 * @subpackage Default_Theme
 */

if ( function_exists('register_sidebar') )
    register_sidebar(array(
        'before_widget' => '<li id="%1$s" class="widget %2$s">',
        'after_widget' => '</li>',
        'before_title' => '<h2 class="widgettitle">',
        'after_title' => '</h2>',
    ));

/** MARCO! */
add_action( 'save_post', 'send_notify_if_editor' );	

//wp_enqueue_script( 'jquery-ui-datepicker' )
/*function my_scripts_method() {
    wp_enqueue_script('jquery-ui-datepicker');            
}    
 
add_action('wp_enqueue_scripts', 'my_scripts_method');*/

/******************** funzione che verifica se si è editore *********************/
function send_notify_if_editor($post_id){
	
	global $wpdb;
	
	$post_status=get_post_status($post_id);
	// controllo che la notifica parta solo se il post è pubblicato, altrimenti viene lanciata anche per bozza ecc..
	if($post_status=='publish'){
		$current_user = wp_get_current_user();
		if ( !empty( $current_user->roles ) && is_array( $current_user->roles ) ) {
			$id_mittente=$current_user->ID;
			foreach ( $current_user->roles as $role )
				if ($role=='admin_editor'){
					// controllo che il post non sia solo una revisione
					if ( !wp_is_post_revision( $post_id ) ) {
						//Riceve la lista completa degli artisti

						$artisti = getListaArtisti("","","",false);
						
						$post_title = get_the_title( $post_id );
						$post_url = get_permalink( $post_id );
						$subject = 'E\' stato inserito un nuovo articolo';

						$message = "E\' stato inserito un nuovo articolo :\n\n";
						$message .= "<a href='". $post_url. "'>" .$post_title. "</a>\n\n";

						foreach($artisti AS $artista){
						/* da re-inserire il foreach lla fine del testing*/
							$wpdb->insert( "messaggi_artisti", 

										   array('data'=>current_time('mysql'),

												 'ora'=>current_time('mysql'),

												 'id_mittente'=>'-2',

												 'id_destinatario'=>$artista->id_utente,
												
												//'id_destinatario'=>'39',

												 'oggetto'=>$post_title, 

												 'testo'=>$message,

												 'letto'=>0) );
							
							// invia notifica
							$destinatarioUser = get_userdata($artista->id_utente);					 
							mailNotificaMessaggio($destinatarioUser->user_email, $role, $destinatarioUser->nickname );
							//wp_mail( $destinatarioUser->user_email, $post_title, $message );
						}
					}
					return;
				}
		}
	}
}

/**** modifiche inserimento utente ****/

add_action( 'show_user_profile', 'save_extra_profile_fields' );
add_action( 'edit_user_profile', 'save_extra_profile_fields' );

function save_extra_profile_fields() {
    
	// controllo il livello dell'utente
	$current_user = wp_get_current_user();
		if ( !empty( $current_user->roles ) && is_array( $current_user->roles ) ) {
			foreach ( $current_user->roles as $role )
				if ($role=='administrator' || $role=='admin_editor'){
					// *** snippet per ottenere l'ID utente del profilo selezionato, se sei admin ***
					global $profileuser;
					$user_id = $profileuser->ID;
					// ******************************************************************************
					$user_meta = get_user_meta($user_id,'Cooperativa',true);
					/*echo "<h3>Cooperativa:  </h3>
						<select name=\"user_coop\">";
						$option_s="<option value=\"Deltartisti\" ";
							if($user_meta=='Deltartisti')
								$option_s.="selected=\"selected\" >".$user_meta."</option>";
							else
								$option_s.=">".$user_meta."</option>";
								
						$option_s.="<option value=\"Terzosuono\" ";
							if($user_meta=='Terzosuono')
								$option_s.="selected=\"selected\" >".$user_meta."</option>";
							else
								$option_s.=">".$user_meta."</option>";
								
						$option_s.="<option value=\"Cooperativacam\" ";
							if($user_meta=='Cooperativacam')
								$option_s.="selected=\"selected\" >".$user_meta."</option>";
							else
								$option_s.=">".$user_meta."</option>";
								
						$option_s.="<option value=\"Cooprcm\" ";
							if($user_meta=='Cooprcm')
								$option_s.="selected=\"selected\" >".$user_meta."</option>";
							else
								$option_s.=">".$user_meta."</option>";
								
						$option_s.="<option value=\"Musicamagica\" ";
							if($user_meta=='Musicamagica')
								$option_s.="selected=\"selected\" >".$user_meta."</option>";
							else
								$option_s.=">".$user_meta."</option>";
						echo "</select>";*/
						echo "<h3>Cooperativa: </h3>".$user_meta;
				}
				else{
					$user_meta = get_user_meta($current_user->ID,'Cooperativa',true);
					echo "<h3>Cooperativa: </h3>".$user_meta;
				}
		}
		
}

// restituisce tutti gli user_id associati alla cooperativa di riferimento
function getListaUserIdPerCooperativa($cooperativa){
	global $wpdb;
	$nome_cooperativa='';
	switch ($cooperativa){
		case "c1" :
			$nome_cooperativa="deltartisti";
			break;
		case "c2" :
			$nome_cooperativa="terzosuono";
			break;
		case "c3" :
			$nome_cooperativa="cooperativacam";
			break;
		case "c4" :
			$nome_cooperativa="cooprcm";
			break;
		case "c5" :
			$nome_cooperativa="musicamagica";
			break;
	}
	$query = "SELECT user_id 
			  FROM wp_usermeta 
			  WHERE meta_key='Cooperativa'  
			  AND meta_value='".$nome_cooperativa."'
			  GROUP BY user_id;";
	$users_id= $wpdb->get_results($query);
	return $users_id;
}

/** @GIACOMO! */
//Restituisce le categorie di artisti in base al padre.
//Per ottenere l'elenco dei padri utilizzare -1
function getListaCategorieArtisti($idPadre)
{
    global $wpdb;
    $query = " SELECT * 
               FROM categorie_artisti
               WHERE id_padre='$idPadre';"; 
    $categorie = $wpdb->get_results($query);
    return $categorie;    
}

//Restituisce una categoria in base all'id
function getCategoriaArtisti($id)
{
    global $wpdb;
    $query = " SELECT * 
               FROM categorie_artisti
               WHERE id='$id' 
			   ORDER BY id_padre;"; 
    $categoria = $wpdb->get_row($query);
    return $categoria;    
}

//Restituisce l'elenco degli artisti che corrispondono ai criteri di ricerca
function getListaArtisti($categoria_ric, $nome_ric, $iniziale_ric, $soloPubblicati)
{
    global $wpdb;

    //Recupera i dati della categoria selezionata per la ricerca
    $categoriaQuery="";
    if ($categoria_ric!='-1' && $categoria_ric!='')
    {
        $categoria = getCategoriaArtisti($categoria_ric);
        if ($categoria->id_padre=='-1')
        {
            //Ricerca in tutte le sottocategorie
            $categoriaQuery = "(";
            $sottocategorie = getListaCategorieArtisti($categoria->id);
            foreach ($sottocategorie as $sottocategoria)
            {
               
               $categoriaQuery .= "(  find_in_set ($sottocategoria->id, pagine.categorie))";
               if (next($sottocategorie)==true) { $categoriaQuery .= ' OR '; };

            }
            $categoriaQuery .= ") AND";
        }
        else
        {
            //Ricerca nella categoria
            $categoriaQuery .= "(  find_in_set ($categoria_ric, pagine.categorie)) AND";
        }
    } 
   
   
    $query = " SELECT * 
               FROM pagine_artisti AS pagine
               WHERE $categoriaQuery
                     (nome LIKE '%$nome_ric%' OR '$nome_ric'='') AND
                     (nome REGEXP '^$iniziale_ric' OR '$iniziale_ric'='') AND
                     (pubblicato = true OR '$soloPubblicati'=false)
               ORDER BY nome;"; 
    $artisti = $wpdb->get_results($query);
    return $artisti;
}

//Restituisce la pagina di un artista a partire dal suo id_utente
function getPaginaArtista($id_utente)
{
    global $wpdb;
    $query = " SELECT * 
               FROM pagine_artisti
               WHERE id_utente=$id_utente;"; 
    $paginaArtista = $wpdb->get_row($query);
    return $paginaArtista;    
}

//Restituisce la lista di tutti gli eventi
function getListaEventi(){
	global $wpdb;
    $query = " SELECT * 
               FROM eventi_artisti 
			   ORDER BY data DESC;"; 
    $eventi = $wpdb->get_results($query);
    return $eventi;  
}

// Restituisce la lista di eventi in base ai criteri di ricerca
function getListaEventiOpt($id_user="", $dopo="", $prima=""){
	global $wpdb;
	
	//echo $id_user." ".$dopo." ".$prima; 
    $query = " SELECT * 
               FROM eventi_artisti
			   WHERE ";

	if($id_user!=""){
		$query.=" id_utente=".$id_user;
		if($dopo!="--"){
			$query.=" AND data >= '".$dopo."'";
			if($prima!="--")
				$query.=" AND data <= '".$prima."' ORDER BY data DESC;";
			else
				$query.=" ORDER BY data DESC;";
		}
		else{
				if ($prima!="--")
					$query.=" AND data <= '".$prima."' ORDER BY data DESC;";
				else
					$query.=" ORDER BY data DESC;";
			}
	}
	elseif($dopo!="--"){
			$query.=" data >= '".$dopo."'";
			if($prima!="--")
				$query.=" AND data <= '".$prima."' ORDER BY data DESC;";
			else
				$query.=" ORDER BY data DESC;";
		}
	else{
			if ($prima!="--")
				$query.=" data <= '".$prima."' ORDER BY data DESC;";
			else
				$query.=" ORDER BY data DESC;";
		}
	
	$wpdb->show_errors();
    $eventi = $wpdb->get_results($query);
	//$wpdb->print_error();
	return $eventi;
}

//Restituisce la lista degli eventi di un determinato artista
function getListaEventiArtista($id_utente)
{
    global $wpdb;
    $query = " SELECT * 
               FROM eventi_artisti
               WHERE id_utente='$id_utente';"; 
    $eventi = $wpdb->get_results($query);
    return $eventi;  
}

//Restituisce gli ultimi 3 eventi inseriti 
function getListaUltimiEventiArtisti()//$id_utente
{
    global $wpdb;
    $query = " SELECT * 
               FROM eventi_artisti
               ORDER BY data DESC,ora DESC
               LIMIT 6;"; 
    $eventi = $wpdb->get_results($query);
    return $eventi;  
}

//Restituisce l'evento di un artista a partire dal suo id (id_utente usato solo per incrementare la sicurezza)
function getEventoArtista($id, $id_utente)
{
    global $wpdb;
    $query = " SELECT * 
               FROM eventi_artisti 
               WHERE (id=$id AND id_utente=$id_utente) "; 
    $evento = $wpdb->get_row($query);
    return $evento;    
}


//Restituisce la lista degli annunci di tutti gli artisti
function getListaAnnunci()
{
    global $wpdb;
    $query = " SELECT * 
               FROM annunci_artisti;"; 
    $annunci = $wpdb->get_results($query);
    return $annunci;  
}//Restituisce gli ultimi 3 annunci inseriti ordinati per data
function getUltimiAnnunci(){	global $wpdb;		$query = " SELECT * 			   FROM  annunci_artisti 			   ORDER BY data DESC			   LIMIT 3;";     $annunci = $wpdb->get_results($query);	return $annunci;}

//Restituisce la lista degli annunci di un determinato artista
function getListaAnnunciArtista($id_utente)
{
    global $wpdb;
    $query = " SELECT * 
               FROM annunci_artisti
               WHERE id_utente='$id_utente';"; 
    $annunci = $wpdb->get_results($query);
    return $annunci;  
}

//Restituisce l'annuncio di un artista a partire dal suo id (id_utente usato solo per incrementare la sicurezza)
function getAnnuncioArtista($id, $id_utente)
{
    global $wpdb;
    $query = " SELECT * 
               FROM annunci_artisti 
               WHERE (id=$id AND id_utente=$id_utente) "; 
    $annuncio = $wpdb->get_row($query);
    return $annuncio;    
}

//Restituisce la lista delle categorie degli annunci
function getListaCategorieAnnunci()
{
    global $wpdb;
    $query = " SELECT * 
               FROM categorie_annunci_artisti"; 
    $categorieAnnunci = $wpdb->get_results($query);
    return $categorieAnnunci;
}

//Restituisce una categoria di annunci in base all'id
function getCategoriaAnnunciArtisti($id)
{
    global $wpdb;
    $query = " SELECT * 
               FROM categorie_annunci_artisti
               WHERE id='$id';"; 
    $categoria = $wpdb->get_row($query);
    return $categoria;    
}

function getRicercaAnnunci($tipologia, $categoria, $parola_chiave, $ricerca_in_descrizione)
{
    global $wpdb;
    if($ricerca_in_descrizione)
    {
        $ricercaParolaQuery = "titolo LIKE '%$parola_chiave%' OR '$parola_chiave'='' OR testo LIKE '%$parola_chiave%'"; 
    }
    else
    {
        $ricercaParolaQuery = "titolo LIKE '%$parola_chiave%' OR '$parola_chiave'=''";
    }
    
    $query = " SELECT * 
               FROM annunci_artisti
               WHERE (tipologia='$tipologia' OR '$tipologia'='') AND
                     (id_categoria='$categoria' OR '$categoria'='') AND
                     ($ricercaParolaQuery)
				ORDER BY data DESC"; 
    $annunci = $wpdb->get_results($query);
    //echo $query;
    echo $ricerca_in_descrizione;
    return $annunci;
}
/*global $wpdb;    $query = " SELECT *                FROM annunci_artisti;";     $annunci = $wpdb->get_results($query);    return $annunci;  */
//Restituisce la lista dei messaggi ricevuti dall'utente
function getListaMessaggiArtista($id_utente)
{
    global $wpdb;
    $query = " SELECT * 
               FROM messaggi_artisti
               WHERE id_destinatario='$id_utente';"; 
    $messaggi = $wpdb->get_results($query);
    return $messaggi;  
}

function countMessaggiArtista($id_utente,$soloNuoviMessaggi)
{
    global $wpdb;
    
    $query = " SELECT COUNT(*) 
               FROM messaggi_artisti
               WHERE (id_destinatario='$id_utente' AND (letto=0 OR '$soloNuoviMessaggi'=false));"; 
    $countMessaggi = $wpdb->get_var( $wpdb->prepare($query) );
    return $countMessaggi;
}

//Restituisce un messaggio di un artista a partire dal suo id (id_utente usato solo per incrementare la sicurezza)
function getMessaggioArtista($id, $id_utente)
{
    global $wpdb;
    $query = " SELECT * 
               FROM messaggi_artisti
               WHERE id='$id' AND id_destinatario='$id_utente';"; 
    $messaggio = $wpdb->get_row($query);
    return $messaggio;  
}

//Invia alla casella e-mail del destinatario una notifica della ricezione del messaggio
function mailNotificaMessaggio($emailTo, $mittenteMsg, $destinatarioMsg)
{
	//Creazione Header delle mail
	//setlocale (LC_ALL, "it_IT");
	$headermail = "From: ArtistiOnline.net <noreply@artistionline.net>\n";
	$headermail .= "X-Mailer: PHP/".phpversion()."\n";
	$headermail .= "MIME-Version: 1.0\n";
	$headermail .= "Content-Type: text/html; charset=UTF-8\n";
	$headermail .= "Content-Transfer-Encoding: 7bit\n\n";
    
    $mail = "<html><body>\r\n".
			"<b>ARTISTIONLINE.NET</b><br/><br/>".
			"<b>Ciao ".$destinatarioMsg.",</b><br/>".
            "Hai ricevuto un messaggio da parte dell'utente <b>".$mittenteMsg."</b>.<br/><br/>".
			"Collegati al sito <a href='".get_bloginfo(url)."'>www.artistionline.net</a> per leggere il contenuto completo del messaggio.<br/>".
            "<br/><br/><br/>".
            "<i>Questo è un messaggio generato automaticamente dal sito www.artistionline.net ; Si prega di non rispondere.</i>".
            "</body></html>";
            
    //elimina eventuali slash introdotti automaticamente dall'uso di caratteri speciali 
	$mail = stripslashes($mail);
    
    if (mail($emailTo, "ArtistiOnline.net - Messaggio ricevuto", $mail, $headermail))
    { /*echo "ok";*/ }
    else 
    { /*echo "no";*/ }     
}  

function download($remoteFilename, $localFilename)
{
    //Ricavo estensione per evitare download di file php
    $ext = strtolower(substr(strrchr($remoteFilename, '.'), 1));
    
    // Effettuo il download
    if ($remoteFilename && file_exists($remoteFilename) && $ext!='php' ) 
    { 
    header("Content-type: Application/octet-stream");
    header("Content-Disposition: attachment; filename=".basename($localFilename));
    header("Content-Length: ".filesize($remoteFilename));
    header("Content-Description: Download");
    header('Content-Transfer-Encoding: binary');
    
    readfile($remoteFilename);  
    } 
    else { echo "<script type='text/javascript'>alert('Impossibile trovare il file.');history.back();</script>"; }
}



// RIMOZIONE USER META CHE NON INTERESSANO

function remove_unwanted_contactmethod( $contactmethods ) {
unset($contactmethods['aim']);
unset($contactmethods['jabber']);
unset($contactmethods['yim']);
return $contactmethods;
}
add_filter('user_contactmethods','remove_unwanted_contactmethod',10,1);

// RIMOZIONE BARRA scelta colore profilo utenti

if(is_admin()){   remove_action("admin_color_scheme_picker", "admin_color_scheme_picker");   add_action( 'personal_options', 'ozh_personal_options'); }

add_filter( 'show_admin_bar', '__return_false' );
add_action( 'admin_print_scripts-profile.php', 'hide_admin_bar_prefs' );
function hide_admin_bar_prefs() { ?>
<style type="text/css">
    .show-admin-bar { display: none; }
</style>
<?php
}

/** @ignore */
function kubrick_head() {
	$head = "<style type='text/css'>\n<!--";
	$output = '';
	if ( kubrick_header_image() ) {
		$url =  kubrick_header_image_url() ;
		$output .= "#header { background: url('$url') no-repeat bottom center; }\n";
	}
	if ( false !== ( $color = kubrick_header_color() ) ) {
		$output .= "#headerimg h1 a, #headerimg h1 a:visited, #headerimg .description { color: $color; }\n";
	}
	if ( false !== ( $display = kubrick_header_display() ) ) {
		$output .= "#headerimg { display: $display }\n";
	}
	$foot = "--></style>\n";
	if ( '' != $output )
		echo $head . $output . $foot;
}

function twentyten_setup() {

	add_theme_support( 'nav-menus' );
	
}
	
register_nav_menus(array('primary'=>__('Primary Menu'),));

add_action('wp_head', 'kubrick_head');

function kubrick_header_image() {
	return apply_filters('kubrick_header_image', get_option('kubrick_header_image'));
}

if (function_exists('add_theme_support')) {
	add_theme_support( 'post-thumbnails' );
	set_post_thumbnail_size( 300, 200, true ); // thumbnail default 
}


function kubrick_upper_color() {
	if (strpos($url = kubrick_header_image_url(), 'header-img.php?') !== false) {
		parse_str(substr($url, strpos($url, '?') + 1), $q);
		return $q['upper'];
	} else
		return '69aee7';
}

function kubrick_lower_color() {
	if (strpos($url = kubrick_header_image_url(), 'header-img.php?') !== false) {
		parse_str(substr($url, strpos($url, '?') + 1), $q);
		return $q['lower'];
	} else
		return '4180b6';
}

function kubrick_header_image_url() {
	if ( $image = kubrick_header_image() )
		$url = get_template_directory_uri() . '/images/' . $image;
	else
		$url = get_template_directory_uri() . '/images/kubrickheader.jpg';

	return $url;
}

function kubrick_header_color() {
	return apply_filters('kubrick_header_color', get_option('kubrick_header_color'));
}

function kubrick_header_color_string() {
	$color = kubrick_header_color();
	if ( false === $color )
		return 'white';

	return $color;
}

function kubrick_header_display() {
	return apply_filters('kubrick_header_display', get_option('kubrick_header_display'));
}

function kubrick_header_display_string() {
	$display = kubrick_header_display();
	return $display ? $display : 'inline';
}

add_action('admin_menu', 'kubrick_add_theme_page');

function kubrick_add_theme_page() {
	if ( isset( $_GET['page'] ) && $_GET['page'] == basename(__FILE__) ) {
		if ( isset( $_REQUEST['action'] ) && 'save' == $_REQUEST['action'] ) {
			check_admin_referer('kubrick-header');
			if ( isset($_REQUEST['njform']) ) {
				if ( isset($_REQUEST['defaults']) ) {
					delete_option('kubrick_header_image');
					delete_option('kubrick_header_color');
					delete_option('kubrick_header_display');
				} else {
					if ( '' == $_REQUEST['njfontcolor'] )
						delete_option('kubrick_header_color');
					else {
						$fontcolor = preg_replace('/^.*(#[0-9a-fA-F]{6})?.*$/', '$1', $_REQUEST['njfontcolor']);
						update_option('kubrick_header_color', $fontcolor);
					}
					if ( preg_match('/[0-9A-F]{6}|[0-9A-F]{3}/i', $_REQUEST['njuppercolor'], $uc) && preg_match('/[0-9A-F]{6}|[0-9A-F]{3}/i', $_REQUEST['njlowercolor'], $lc) ) {
						$uc = ( strlen($uc[0]) == 3 ) ? $uc[0]{0}.$uc[0]{0}.$uc[0]{1}.$uc[0]{1}.$uc[0]{2}.$uc[0]{2} : $uc[0];
						$lc = ( strlen($lc[0]) == 3 ) ? $lc[0]{0}.$lc[0]{0}.$lc[0]{1}.$lc[0]{1}.$lc[0]{2}.$lc[0]{2} : $lc[0];
						update_option('kubrick_header_image', "header-img.php?upper=$uc&lower=$lc");
					}

					if ( isset($_REQUEST['toggledisplay']) ) {
						if ( false === get_option('kubrick_header_display') )
							update_option('kubrick_header_display', 'none');
						else
							delete_option('kubrick_header_display');
					}
				}
			} else {

				if ( isset($_REQUEST['headerimage']) ) {
					check_admin_referer('kubrick-header');
					if ( '' == $_REQUEST['headerimage'] )
						delete_option('kubrick_header_image');
					else {
						$headerimage = preg_replace('/^.*?(header-img.php\?upper=[0-9a-fA-F]{6}&lower=[0-9a-fA-F]{6})?.*$/', '$1', $_REQUEST['headerimage']);
						update_option('kubrick_header_image', $headerimage);
					}
				}

				if ( isset($_REQUEST['fontcolor']) ) {
					check_admin_referer('kubrick-header');
					if ( '' == $_REQUEST['fontcolor'] )
						delete_option('kubrick_header_color');
					else {
						$fontcolor = preg_replace('/^.*?(#[0-9a-fA-F]{6})?.*$/', '$1', $_REQUEST['fontcolor']);
						update_option('kubrick_header_color', $fontcolor);
					}
				}

				if ( isset($_REQUEST['fontdisplay']) ) {
					check_admin_referer('kubrick-header');
					if ( '' == $_REQUEST['fontdisplay'] || 'inline' == $_REQUEST['fontdisplay'] )
						delete_option('kubrick_header_display');
					else
						update_option('kubrick_header_display', 'none');
				}
			}
			//print_r($_REQUEST);
			wp_redirect("themes.php?page=functions.php&saved=true");
			die;
		}
		add_action('admin_head', 'kubrick_theme_page_head');
	}
	add_theme_page(__('Customize Header'), __('Header Image and Color'), 'edit_themes', basename(__FILE__), 'kubrick_theme_page');
}

function kubrick_theme_page_head() {
?>
<script type="text/javascript" src="../wp-includes/js/colorpicker.js"></script>
<script type='text/javascript'>
// <![CDATA[
	function pickColor(color) {
		ColorPicker_targetInput.value = color;
		kUpdate(ColorPicker_targetInput.id);
	}
	function PopupWindow_populate(contents) {
		contents += '<br /><p style="text-align:center;margin-top:0px;"><input type="button" class="button-secondary" value="<?php echo attribute_escape(__('Close Color Picker')); ?>" onclick="cp.hidePopup(\'prettyplease\')"></input></p>';
		this.contents = contents;
		this.populated = false;
	}
	function PopupWindow_hidePopup(magicword) {
		if ( magicword != 'prettyplease' )
			return false;
		if (this.divName != null) {
			if (this.use_gebi) {
				document.getElementById(this.divName).style.visibility = "hidden";
			}
			else if (this.use_css) {
				document.all[this.divName].style.visibility = "hidden";
			}
			else if (this.use_layers) {
				document.layers[this.divName].visibility = "hidden";
			}
		}
		else {
			if (this.popupWindow && !this.popupWindow.closed) {
				this.popupWindow.close();
				this.popupWindow = null;
			}
		}
		return false;
	}
	function colorSelect(t,p) {
		if ( cp.p == p && document.getElementById(cp.divName).style.visibility != "hidden" )
			cp.hidePopup('prettyplease');
		else {
			cp.p = p;
			cp.select(t,p);
		}
	}
	function PopupWindow_setSize(width,height) {
		this.width = 162;
		this.height = 210;
	}

	var cp = new ColorPicker();
	function advUpdate(val, obj) {
		document.getElementById(obj).value = val;
		kUpdate(obj);
	}
	function kUpdate(oid) {
		if ( 'uppercolor' == oid || 'lowercolor' == oid ) {
			uc = document.getElementById('uppercolor').value.replace('#', '');
			lc = document.getElementById('lowercolor').value.replace('#', '');
			hi = document.getElementById('headerimage');
			hi.value = 'header-img.php?upper='+uc+'&lower='+lc;
			document.getElementById('header').style.background = 'url("<?php echo get_template_directory_uri(); ?>/images/'+hi.value+'") center no-repeat';
			document.getElementById('advuppercolor').value = '#'+uc;
			document.getElementById('advlowercolor').value = '#'+lc;
		}
		if ( 'fontcolor' == oid ) {
			document.getElementById('header').style.color = document.getElementById('fontcolor').value;
			document.getElementById('advfontcolor').value = document.getElementById('fontcolor').value;
		}
		if ( 'fontdisplay' == oid ) {
			document.getElementById('headerimg').style.display = document.getElementById('fontdisplay').value;
		}
	}
	function toggleDisplay() {
		td = document.getElementById('fontdisplay');
		td.value = ( td.value == 'none' ) ? 'inline' : 'none';
		kUpdate('fontdisplay');
	}
	function toggleAdvanced() {
		a = document.getElementById('jsAdvanced');
		if ( a.style.display == 'none' )
			a.style.display = 'block';
		else
			a.style.display = 'none';
	}
	function kDefaults() {
		document.getElementById('headerimage').value = '';
		document.getElementById('advuppercolor').value = document.getElementById('uppercolor').value = '#69aee7';
		document.getElementById('advlowercolor').value = document.getElementById('lowercolor').value = '#4180b6';
		document.getElementById('header').style.background = 'url("<?php echo get_template_directory_uri(); ?>/images/kubrickheader.jpg") center no-repeat';
		document.getElementById('header').style.color = '#FFFFFF';
		document.getElementById('advfontcolor').value = document.getElementById('fontcolor').value = '';
		document.getElementById('fontdisplay').value = 'inline';
		document.getElementById('headerimg').style.display = document.getElementById('fontdisplay').value;
	}
	function kRevert() {
		document.getElementById('headerimage').value = '<?php echo js_escape(kubrick_header_image()); ?>';
		document.getElementById('advuppercolor').value = document.getElementById('uppercolor').value = '#<?php echo js_escape(kubrick_upper_color()); ?>';
		document.getElementById('advlowercolor').value = document.getElementById('lowercolor').value = '#<?php echo js_escape(kubrick_lower_color()); ?>';
		document.getElementById('header').style.background = 'url("<?php echo js_escape(kubrick_header_image_url()); ?>") center no-repeat';
		document.getElementById('header').style.color = '';
		document.getElementById('advfontcolor').value = document.getElementById('fontcolor').value = '<?php echo js_escape(kubrick_header_color_string()); ?>';
		document.getElementById('fontdisplay').value = '<?php echo js_escape(kubrick_header_display_string()); ?>';
		document.getElementById('headerimg').style.display = document.getElementById('fontdisplay').value;
	}
	function kInit() {
		document.getElementById('jsForm').style.display = 'block';
		document.getElementById('nonJsForm').style.display = 'none';
	}
	addLoadEvent(kInit);
// ]]>
</script>
<style type='text/css'>
	#headwrap {
		text-align: center;
	}
	#kubrick-header {
		font-size: 80%;
	}
	#kubrick-header .hibrowser {
		width: 780px;
		height: 260px;
		overflow: scroll;
	}
	#kubrick-header #hitarget {
		display: none;
	}
	#kubrick-header #header h1 {
		font-family: 'Trebuchet MS', 'Lucida Grande', Verdana, Arial, Sans-Serif;
		font-weight: bold;
		font-size: 4em;
		text-align: center;
		padding-top: 70px;
		margin: 0;
	}

	#kubrick-header #header .description {
		font-family: 'Lucida Grande', Verdana, Arial, Sans-Serif;
		font-size: 1.2em;
		text-align: center;
	}
	#kubrick-header #header {
		text-decoration: none;
		color: <?php echo kubrick_header_color_string(); ?>;
		padding: 0;
		margin: 0;
		height: 200px;
		text-align: center;
		background: url('<?php echo kubrick_header_image_url(); ?>') center no-repeat;
	}
	#kubrick-header #headerimg {
		margin: 0;
		height: 200px;
		width: 100%;
		display: <?php echo kubrick_header_display_string(); ?>;
	}
	#jsForm {
		display: none;
		text-align: center;
	}
	#jsForm input.submit, #jsForm input.button, #jsAdvanced input.button {
		padding: 0px;
		margin: 0px;
	}
	#advanced {
		text-align: center;
		width: 620px;
	}
	html>body #advanced {
		text-align: center;
		position: relative;
		left: 50%;
		margin-left: -380px;
	}
	#jsAdvanced {
		text-align: right;
	}
	#nonJsForm {
		position: relative;
		text-align: left;
		margin-left: -370px;
		left: 50%;
	}
	#nonJsForm label {
		padding-top: 6px;
		padding-right: 5px;
		float: left;
		width: 100px;
		text-align: right;
	}
	.defbutton {
		font-weight: bold;
	}
	.zerosize {
		width: 0px;
		height: 0px;
		overflow: hidden;
	}
	#colorPickerDiv a, #colorPickerDiv a:hover {
		padding: 1px;
		text-decoration: none;
		border-bottom: 0px;
	}
</style>
<?php
}

function kubrick_theme_page() {
	if ( isset( $_REQUEST['saved'] ) ) echo '<div id="message" class="updated fade"><p><strong>'.__('Options saved.').'</strong></p></div>';
?>
<div class='wrap'>
	<h2><?php _e('Customize Header'); ?></h2>
	<div id="kubrick-header">
		<div id="headwrap">
			<div id="header">
				<div id="headerimg">
					<h1><?php bloginfo('name'); ?></h1>
					<div class="description"><?php bloginfo('description'); ?></div>
				</div>
			</div>
		</div>
		<br />
		<div id="nonJsForm">
			<form method="post" action="">
				<?php wp_nonce_field('kubrick-header'); ?>
				<div class="zerosize"><input type="submit" name="defaultsubmit" value="<?php echo attribute_escape(__('Save')); ?>" /></div>
					<label for="njfontcolor"><?php _e('Font Color:'); ?></label><input type="text" name="njfontcolor" id="njfontcolor" value="<?php echo attribute_escape(kubrick_header_color()); ?>" /> <?php printf(__('Any CSS color (%s or %s or %s)'), '<code>red</code>', '<code>#FF0000</code>', '<code>rgb(255, 0, 0)</code>'); ?><br />
					<label for="njuppercolor"><?php _e('Upper Color:'); ?></label><input type="text" name="njuppercolor" id="njuppercolor" value="#<?php echo attribute_escape(kubrick_upper_color()); ?>" /> <?php printf(__('HEX only (%s or %s)'), '<code>#FF0000</code>', '<code>#F00</code>'); ?><br />
				<label for="njlowercolor"><?php _e('Lower Color:'); ?></label><input type="text" name="njlowercolor" id="njlowercolor" value="#<?php echo attribute_escape(kubrick_lower_color()); ?>" /> <?php printf(__('HEX only (%s or %s)'), '<code>#FF0000</code>', '<code>#F00</code>'); ?><br />
				<input type="hidden" name="hi" id="hi" value="<?php echo attribute_escape(kubrick_header_image()); ?>" />
				<input type="submit" name="toggledisplay" id="toggledisplay" value="<?php echo attribute_escape(__('Toggle Text')); ?>" />
				<input type="submit" name="defaults" value="<?php echo attribute_escape(__('Use Defaults')); ?>" />
				<input type="submit" class="defbutton" name="submitform" value="&nbsp;&nbsp;<?php _e('Save'); ?>&nbsp;&nbsp;" />
				<input type="hidden" name="action" value="save" />
				<input type="hidden" name="njform" value="true" />
			</form>
		</div>
		<div id="jsForm">
			<form style="display:inline;" method="post" name="hicolor" id="hicolor" action="<?php echo attribute_escape($_SERVER['REQUEST_URI']); ?>">
				<?php wp_nonce_field('kubrick-header'); ?>
	<input type="button"  class="button-secondary" onclick="tgt=document.getElementById('fontcolor');colorSelect(tgt,'pick1');return false;" name="pick1" id="pick1" value="<?php echo attribute_escape(__('Font Color')); ?>"></input>
		<input type="button" class="button-secondary" onclick="tgt=document.getElementById('uppercolor');colorSelect(tgt,'pick2');return false;" name="pick2" id="pick2" value="<?php echo attribute_escape(__('Upper Color')); ?>"></input>
		<input type="button" class="button-secondary" onclick="tgt=document.getElementById('lowercolor');colorSelect(tgt,'pick3');return false;" name="pick3" id="pick3" value="<?php echo attribute_escape(__('Lower Color')); ?>"></input>
				<input type="button" class="button-secondary" name="revert" value="<?php echo attribute_escape(__('Revert')); ?>" onclick="kRevert()" />
				<input type="button" class="button-secondary" value="<?php echo attribute_escape(__('Advanced')); ?>" onclick="toggleAdvanced()" />
				<input type="hidden" name="action" value="save" />
				<input type="hidden" name="fontdisplay" id="fontdisplay" value="<?php echo attribute_escape(kubrick_header_display()); ?>" />
				<input type="hidden" name="fontcolor" id="fontcolor" value="<?php echo attribute_escape(kubrick_header_color()); ?>" />
				<input type="hidden" name="uppercolor" id="uppercolor" value="<?php echo attribute_escape(kubrick_upper_color()); ?>" />
				<input type="hidden" name="lowercolor" id="lowercolor" value="<?php echo attribute_escape(kubrick_lower_color()); ?>" />
				<input type="hidden" name="headerimage" id="headerimage" value="<?php echo attribute_escape(kubrick_header_image()); ?>" />
				<p class="submit"><input type="submit" name="submitform" class="button-primary" value="<?php echo attribute_escape(__('Update Header')); ?>" onclick="cp.hidePopup('prettyplease')" /></p>
			</form>
			<div id="colorPickerDiv" style="z-index: 100;background:#eee;border:1px solid #ccc;position:absolute;visibility:hidden;"> </div>
			<div id="advanced">
				<form id="jsAdvanced" style="display:none;" action="">
					<?php wp_nonce_field('kubrick-header'); ?>
					<label for="advfontcolor"><?php _e('Font Color (CSS):'); ?> </label><input type="text" id="advfontcolor" onchange="advUpdate(this.value, 'fontcolor')" value="<?php echo attribute_escape(kubrick_header_color()); ?>" /><br />
					<label for="advuppercolor"><?php _e('Upper Color (HEX):');?> </label><input type="text" id="advuppercolor" onchange="advUpdate(this.value, 'uppercolor')" value="#<?php echo attribute_escape(kubrick_upper_color()); ?>" /><br />
					<label for="advlowercolor"><?php _e('Lower Color (HEX):'); ?> </label><input type="text" id="advlowercolor" onchange="advUpdate(this.value, 'lowercolor')" value="#<?php echo attribute_escape(kubrick_lower_color()); ?>" /><br />
					<input type="button" class="button-secondary" name="default" value="<?php echo attribute_escape(__('Select Default Colors')); ?>" onclick="kDefaults()" /><br />
					<input type="button" class="button-secondary" onclick="toggleDisplay();return false;" name="pick" id="pick" value="<?php echo attribute_escape(__('Toggle Text Display')); ?>"></input><br />
				</form>
			</div>
		</div>
	</div>
</div>
<?php } ?>
